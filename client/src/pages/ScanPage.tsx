...
export function ScanPage() {
  ...
  return (
    <div className="animate-fade-up mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-10">
      ...
      {error && (
        <div className="animate-fade-in flex items-start gap-3 rounded-2xl bg-[#fff0f0] p-4 text-[#b13030]">
          <AlertCircle size={20} className="mt-0.5 shrink-0" />
          <p className="text-sm font-medium">{error}</p>
        </div>
      )}

      <div className="relative aspect-square w-full overflow-hidden rounded-3xl bg-gradient-to-br from-primary-100 to-primary-200/60 sm:aspect-[4/3]">
        {mode === 'camera' ? (
          <>
            <div id={containerId} className="absolute inset-0" />
            {/* Scan line animation overlay */}
            <div className="pointer-events-none absolute inset-x-0 top-1/4 h-1/2">
              <div className="animate-scan-line mx-auto h-px w-1/2 bg-status-success shadow-[0_0_12px_3px_#56bb69]" />
            </div>
          </>
        ) : (
          <div className="absolute inset-0 grid place-items-center">
            <div className="flex flex-col items-center gap-4 text-center">
              <div className="animate-pulse-soft grid h-20 w-20 place-items-center rounded-3xl bg-white text-primary-700 shadow-floating">
                <ScanLine size={40} />
              </div>
              <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <button
          ...
          className={cn(
            ...
            mode === 'camera'
              ? 'bg-status-error text-white shadow-cta hover:brightness-95 hover:-translate-y-0.5'
              : 'bg-primary-500 text-white shadow-cta hover:bg-primary-600 hover:-translate-y-0.5 hover:shadow-floating',
          )}
        >
          <Camera size={20} />
      ...
      {mode === 'manual' && (
        ...
        >
          <input
            ...
            className="flex-1 rounded-2xl bg-white px-5 py-4 text-base font-semibold tracking-widest text-neutral-900 shadow-card ring-1 ring-neutral-100 placeholder:font-medium placeholder:tracking-widest placeholder:text-neutral-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-400"
          />
          <button
            type="submit"
            className="rounded-full bg-primary-500 px-6 py-4 text-base font-semibold text-white shadow-cta transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary-600 hover:shadow-floating active:translate-y-0 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-primary-400 focus-visible:ring-offset-2"
          >
            Buscar pedido
          </button>
        </form>
...
function CornerBrackets() {
  const base =
    'pointer-events-none absolute h-8 w-8 border-white/70'
  return (
    <>
