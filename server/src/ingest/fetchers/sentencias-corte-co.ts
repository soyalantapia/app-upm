...
// Portado de client/src/lib/sources/sentencias-corte-co.ts — mismos ids
// (co-corte-full-<slug>) para que el dedupe del front colapse server/cliente.
//
...
//
// Diferencia con corte-constitucional-co: ese trae solo 191 sentencias
...
export async function fetchSentenciasCorteCO(opts?: { limit?: number }): Promise<NewsItem[]> {
  const limit = opts?.limit ?? 50
  const params = new URLSearchParams({
    $limit: String(limit),
    $order: 'fecha_sentencia DESC',
  })
  const data = await fetchJson<SentenciaCompletaRow[]>(`${ENDPOINT}?${params.toString()}`)
...
function mapSentencia(r: SentenciaCompletaRow): NewsItem | null {
  ...
  return {
    ...
    excerpt: truncateExcerpt(excerptParts),
