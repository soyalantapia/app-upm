...
// Portado de client/src/lib/sources/presidencia-co.ts — mismos ids (co-pres-<tipo>-<slug>)
...
//
// Server-side: fetchJson directo a datos.gov.co, sin proxies CORS.
...
export async function fetchPresidenciaColombia(opts?: {
  ...
  const data = await fetchJson<NormaRow[]>(`${ENDPOINT}?${params.toString()}`)
...
export function fetchLeyesPresidenciaColombia(opts?: { limit?: number }): Promise<NewsItem[]> {
...
export function fetchDecretosPresidenciaColombia(opts?: { limit?: number }): Promise<NewsItem[]> {
  return fetchPresidenciaColombia({
    limit: opts?.limit ?? 40,
...
function mapNorma(r: NormaRow): NewsItem | null {
  ...
  // ID estable: tipo + slug del título (idéntico al cliente)
  const slug = titulo
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
  ...
  return {
    ...
    excerpt: truncateExcerpt(descripcion || titulo),
