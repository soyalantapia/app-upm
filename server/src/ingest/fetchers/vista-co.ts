...
// Portado de client/src/lib/sources/vista-co.ts — mismos ids (co-vista-<código>)
...
export async function fetchVistaProyectosColombia(opts?: { limit?: number }): Promise<NewsItem[]> {
  // El cliente llama con { limit: 50 } desde FETCHERS — mismo default acá.
  ...
  const data = await fetchJson<VistaRow[]>(`${ENDPOINT}?${params.toString()}`)
...
function mapVista(r: VistaRow): NewsItem | null {
  ...
  const excerptCut = truncateExcerpt(excerptParts.join(' · '))
