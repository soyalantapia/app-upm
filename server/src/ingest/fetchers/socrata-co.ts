...
// Portado de client/src/lib/sources/socrata-co.ts — mismo mapeo → ids idénticos
// (co-proyecto-<num>, co-ley-<num>) para que el dedupe del front colapse
...
// Server-side: fetchJson directo (sin proxy CORS, timeout 15s incluido).
//
// Schema:
...
function detectRelevance(estado: string): Relevance {
...
export async function fetchProyectosColombia(opts?: { limit?: number }): Promise<NewsItem[]> {
  ...
  const data = await fetchJson<SenadoCoRow[]>(`${ENDPOINT}?${params.toString()}`)
...
export async function fetchLeyesColombia(opts?: { limit?: number }): Promise<NewsItem[]> {
