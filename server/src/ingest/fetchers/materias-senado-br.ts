...
// Portado de client/src/lib/sources/materias-senado-br.ts → ids idénticos
// (br-senado-mat-<sigla>-<numero>-<ano>) para que el dedupe del front colapse
// server/cliente en uno. Server-side: fetchJson directo, sin proxy CORS.
...
export async function fetchMateriasSenadoBR(opts?: { limit?: number }): Promise<NewsItem[]> {
...
function mapRow(r: MateriaRow): NewsItem | null {
  ...
  return {
    ...
    excerpt: ementa ? truncateExcerpt(ementa) : `Matéria ${tipoDoc} em tramitação no Senado Federal.`,
