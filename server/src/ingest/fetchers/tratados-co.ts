...
// Portado de client/src/lib/sources/tratados-co.ts — mismo mapeo → ids idénticos
// (co-tratado-<fecha>-<slug>) para que el dedupe del front colapse server/cliente.
// Server-side: fetchJson directo (sin proxy CORS).
...
function detectRelevance(naturaleza: string | undefined, vigente: string | undefined): Relevance {
...
export async function fetchTratadosColombia(opts?: { limit?: number }): Promise<NewsItem[]> {
  ...
  const data = await fetchJson<TratadoRow[]>(`${ENDPOINT}?${params.toString()}`)
