...
// Portado de client/src/lib/sources/votacoes-br.ts — mismo mapeo → ids idénticos
// (br-votacao-<id>) para que el dedupe del front colapse server/cliente en uno.
//
// Câmara dos Deputados de Brasil · Votaciones recientes.
// API pública: dadosabertos.camara.leg.br/api/v2/votacoes — server-side, sin proxy CORS.
//
...
export async function fetchVotacoesCamaraBR(opts?: { limit?: number }): Promise<NewsItem[]> {
  ...
  const json = await fetchJson<{ dados?: VotacaoRow[] }>(`${BASE}/votacoes?${params.toString()}`)
